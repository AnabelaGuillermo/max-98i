...
document.addEventListener('DOMContentLoaded', () => {
    const resultadosCrimen = document.getElementById('resultados-categorias-crimen');
    let peliculas = obtenerPeliculasDeLS().filter(pelicula => pelicula.categoria === 'Crimen' && pelicula.publicada === 'Sí');
    ...
    function cargarPeliculasSeries() {
        ...
        if (moviesToLoad.length === 0 && currentPage === 0) {
            ...
            resultadosCrimen.appendChild(mensajeNoResultados);
            return;
        }
        ...
        moviesToLoad.forEach(pelicula => {
            ...
            col.appendChild(img);
            col.appendChild(h5);
            resultadosCrimen.appendChild(col);
        });
