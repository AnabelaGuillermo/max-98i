...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let peliculas = obtenerPeliculasDeLS().filter(pelicula => pelicula.categoria === 'Terror' && pelicula.publicada === 'Sí');
    ...
    function cargarPeliculasSeries() {
        ...
        if (moviesToLoad.length === 0 && currentPage === 0) {
            ...
            resultadosTerror.appendChild(mensajeNoResultados);
            return;
        }
        ...
        moviesToLoad.forEach(pelicula => {
            ...
            col.appendChild(img);
            col.appendChild(h5);
            resultadosTerror.appendChild(col);
        });
